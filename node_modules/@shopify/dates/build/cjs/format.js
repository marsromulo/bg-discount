...
function format(date, fmtStr, tz, localeStr) {
  const {
    locale,
    timeZone
  } = formatDate.memoizedGetDateTimeFormat(localeStr, {
    timeZone: tz
  }).resolvedOptions();
  return fmtStr.replace(ALL_TOKENS_REGEX, token => {
    ...
    const timeUnit = (_Intl$DateTimeFormat$ = Intl.DateTimeFormat(locale, _rollupPluginBabelHelpers.objectSpread2({
      timeZone
    }, options)).formatToParts(date).find(part => part.type === type)) === null || _Intl$DateTimeFormat$ === void 0 ? void 0 : _Intl$DateTimeFormat$.value;
