...
const USER_INPUT_TIME_REGEX = /^(\d{1,2})[:\s]?(\d{1,2})?[:-\s]?(?:\d{1,2})?(\s?[APap][Mm])?/;
/**
...
function generateTimes(startDate, disableTimesBefore) {
  if (disableTimesBefore && dates.isSameDay(startDate, disableTimesBefore)) {
...
function formatDateListAsOptionList(dates$1, locale, ianaTimezone) {
  return dates$1.map(date => ({
    value: dates.applyTimeZoneOffset(date, ianaTimezone).toISOString(),
...
function getLocalizedTimeForDate(date, locale) {
...
function getValidDateForTime(timeInput, forDate, ianaTimezone) {
  if (isValidTime(timeInput)) {
    ...
    nextDate.setHours(buildNextDateHour(hours, period, dates$1.getBrowserAndShopTimeZoneOffset(ianaTimezone)), Number(minutes));
    return nextDate;
  }
...
function buildNextDateHour(hours, period, browserAndShopTimeZoneOffset) {
  ...
  const numberHours = Number(hours); // User entered time in the shop's timezone, but `new Date()` uses the browser's timezone.

  const hoursWithTimeZoneOffset = numberHours + browserAndShopTimeZoneOffset; // When `hours` is 12PM (noon), adding 12 hours would set the time to
...
exports.formatDateListAsOptionList = formatDateListAsOptionList;
exports.generateTimes = generateTimes;
exports.getLocalizedTimeForDate = getLocalizedTimeForDate;
exports.getValidDateForTime = getValidDateForTime;
