export declare enum Weekday {
    Sunday = 0,
    Monday = 1,
    Tuesday = 2,
    Wednesday = 3,
    Thursday = 4,
    Friday = 5,
    Saturday = 6
}
export declare const DEFAULT_WEEK_START_DAY = Weekday.Sunday;
export declare enum DiscountMethod {
    Code = "Code",
    Automatic = "Automatic"
}
export declare enum DiscountClass {
    Product = "PRODUCT",
    Order = "ORDER",
    Shipping = "SHIPPING"
}
export declare const DEFAULT_DISCOUNT_CODE_LENGTH = 12;
export declare enum RecurringPaymentType {
    FirstPayment = "FIRST_PAYMENT",
    MultiplePayments = "MULTIPLE_PAYMENTS",
    AllPayments = "ALL_PAYMENTS"
}
export declare enum RequirementType {
    None = "NONE",
    Subtotal = "SUBTOTAL",
    Quantity = "QUANTITY"
}
export declare enum AppliesTo {
    Order = "ORDER",
    Products = "PRODUCTS",
    Collections = "COLLECTIONS"
}
export declare enum CountrySelectionType {
    AllCountries = "ALL_COUNTRIES",
    SelectedCountries = "SELECTED_COUNTRIES"
}
export declare enum DiscountStatus {
    Active = "ACTIVE",
    Expired = "EXPIRED",
    Scheduled = "SCHEDULED"
}
export declare enum PurchaseType {
    OneTimePurchase = "ONE_TIME_PURCHASE",
    Subscription = "SUBSCRIPTION",
    Both = "BOTH"
}
export declare enum Eligibility {
    Everyone = "EVERYONE",
    Customers = "CUSTOMERS",
    CustomerSegments = "CUSTOMER_SEGMENTS"
}
export declare enum SupportedCountryCode {
    Af = "AF",
    Ax = "AX",
    Al = "AL",
    Dz = "DZ",
    Ad = "AD",
    Ao = "AO",
    Ai = "AI",
    Ag = "AG",
    Ar = "AR",
    Am = "AM",
    Aw = "AW",
    Ac = "AC",
    Au = "AU",
    At = "AT",
    Az = "AZ",
    Bs = "BS",
    Bh = "BH",
    Bd = "BD",
    Bb = "BB",
    By = "BY",
    Be = "BE",
    Bz = "BZ",
    Bj = "BJ",
    Bm = "BM",
    Bt = "BT",
    Bo = "BO",
    Ba = "BA",
    Bw = "BW",
    Bv = "BV",
    Br = "BR",
    Io = "IO",
    Bn = "BN",
    Bg = "BG",
    Bf = "BF",
    Bi = "BI",
    Kh = "KH",
    Ca = "CA",
    Cv = "CV",
    Bq = "BQ",
    Ky = "KY",
    Cf = "CF",
    Td = "TD",
    Cl = "CL",
    Cn = "CN",
    Cx = "CX",
    Cc = "CC",
    Co = "CO",
    Km = "KM",
    Cg = "CG",
    Cd = "CD",
    Ck = "CK",
    Cr = "CR",
    Hr = "HR",
    Cu = "CU",
    Cw = "CW",
    Cy = "CY",
    Cz = "CZ",
    Ci = "CI",
    Dk = "DK",
    Dj = "DJ",
    Dm = "DM",
    Do = "DO",
    Ec = "EC",
    Eg = "EG",
    Sv = "SV",
    Gq = "GQ",
    Er = "ER",
    Ee = "EE",
    Sz = "SZ",
    Et = "ET",
    Fk = "FK",
    Fo = "FO",
    Fj = "FJ",
    Fi = "FI",
    Fr = "FR",
    Gf = "GF",
    Pf = "PF",
    Tf = "TF",
    Ga = "GA",
    Gm = "GM",
    Ge = "GE",
    De = "DE",
    Gh = "GH",
    Gi = "GI",
    Gr = "GR",
    Gl = "GL",
    Gd = "GD",
    Gp = "GP",
    Gt = "GT",
    Gg = "GG",
    Gn = "GN",
    Gw = "GW",
    Gy = "GY",
    Ht = "HT",
    Hm = "HM",
    Va = "VA",
    Hn = "HN",
    Hk = "HK",
    Hu = "HU",
    Is = "IS",
    In = "IN",
    Id = "ID",
    Ir = "IR",
    Iq = "IQ",
    Ie = "IE",
    Im = "IM",
    Il = "IL",
    It = "IT",
    Jm = "JM",
    Jp = "JP",
    Je = "JE",
    Jo = "JO",
    Kz = "KZ",
    Ke = "KE",
    Ki = "KI",
    Kp = "KP",
    Xk = "XK",
    Kw = "KW",
    Kg = "KG",
    La = "LA",
    Lv = "LV",
    Lb = "LB",
    Ls = "LS",
    Lr = "LR",
    Ly = "LY",
    Li = "LI",
    Lt = "LT",
    Lu = "LU",
    Mo = "MO",
    Mg = "MG",
    Mw = "MW",
    My = "MY",
    Mv = "MV",
    Ml = "ML",
    Mt = "MT",
    Mq = "MQ",
    Mr = "MR",
    Mu = "MU",
    Yt = "YT",
    Mx = "MX",
    Md = "MD",
    Mc = "MC",
    Mn = "MN",
    Me = "ME",
    Ms = "MS",
    Ma = "MA",
    Mz = "MZ",
    Mm = "MM",
    Na = "NA",
    Nr = "NR",
    Np = "NP",
    Nl = "NL",
    An = "AN",
    Nc = "NC",
    Nz = "NZ",
    Ni = "NI",
    Ne = "NE",
    Ng = "NG",
    Nu = "NU",
    Nf = "NF",
    Mk = "MK",
    No = "NO",
    Om = "OM",
    Pk = "PK",
    Ps = "PS",
    Pa = "PA",
    Pg = "PG",
    Py = "PY",
    Pe = "PE",
    Ph = "PH",
    Pn = "PN",
    Pl = "PL",
    Pt = "PT",
    Qa = "QA",
    Cm = "CM",
    Re = "RE",
    Ro = "RO",
    Ru = "RU",
    Rw = "RW",
    Bl = "BL",
    Sh = "SH",
    Kn = "KN",
    Lc = "LC",
    Mf = "MF",
    Pm = "PM",
    Ws = "WS",
    Sm = "SM",
    St = "ST",
    Sa = "SA",
    Sn = "SN",
    Rs = "RS",
    Sc = "SC",
    Sl = "SL",
    Sg = "SG",
    Sx = "SX",
    Sk = "SK",
    Si = "SI",
    Sb = "SB",
    So = "SO",
    Za = "ZA",
    Gs = "GS",
    Kr = "KR",
    Ss = "SS",
    Es = "ES",
    Lk = "LK",
    Vc = "VC",
    Sd = "SD",
    Sr = "SR",
    Sj = "SJ",
    Se = "SE",
    Ch = "CH",
    Sy = "SY",
    Tw = "TW",
    Tj = "TJ",
    Tz = "TZ",
    Th = "TH",
    Tl = "TL",
    Tg = "TG",
    Tk = "TK",
    To = "TO",
    Tt = "TT",
    Ta = "TA",
    Tn = "TN",
    Tr = "TR",
    Tm = "TM",
    Tc = "TC",
    Tv = "TV",
    Ug = "UG",
    Ua = "UA",
    Ae = "AE",
    Gb = "GB",
    Us = "US",
    Um = "UM",
    Uy = "UY",
    Uz = "UZ",
    Vu = "VU",
    Ve = "VE",
    Vn = "VN",
    Vg = "VG",
    Wf = "WF",
    Eh = "EH",
    Ye = "YE",
    Zm = "ZM",
    Zw = "ZW",
    Zz = "ZZ"
}
export declare const REST_OF_WORLD = "REST_OF_WORLD";
