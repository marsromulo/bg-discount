...
import { translationsImport, i18nGeneratedDictionaryCallExpression, fallbackTranslationsImport, i18nCallExpression } from './babel-templates.mjs';
import { DEFAULT_FALLBACK_LOCALE, TRANSLATION_DIRECTORY_NAME } from './shared.mjs';
...
function injectWithI18nArguments({
  ...
  function addI18nArguments({
    ...
    const translationFilePaths = getTranslationFilePaths(filename, TRANSLATION_DIRECTORY_NAME);
    ...
    visitor: {
      ...
      ImportDeclaration(nodePath, state) {
        ...
        specifiers.forEach(specifier => {
          ...
          const fallbackLocale = defaultLocale ? defaultLocale : DEFAULT_FALLBACK_LOCALE;
          const fallbackID = nodePath.scope.generateUidIdentifier(camelCase(fallbackLocale)).name;
          const {
            filename
          } = this.file.opts;
          ...
          if (mode === 'from-dictionary-index') {
            const _translationArrayID = '__shopify__i18n_translations';
            addI18nArguments({
              binding,
              bindingName,
              filename,
              ...
              insertImport() {
                ...
                program.node.body.unshift(translationsImport(template, {
                  id: _translationArrayID
                }));
              },

              rewritei18nCall(referencePathToRewrite) {
                referencePathToRewrite.parentPath.replaceWith(i18nGeneratedDictionaryCallExpression(template, {
                  id: generateID(filename),
                  translationsID: _translationArrayID,
                  bindingName
                }));
              }
          ...
          addI18nArguments({
            ...
            insertImport() {
              ...
              program.node.body.unshift(fallbackTranslationsImport(template, {
                id: fallbackID,
                fallbackLocale
              }));
            ...
            rewritei18nCall(referencePathToRewrite, translationFilePaths) {
              referencePathToRewrite.parentPath.replaceWith(i18nCallExpression(template, {
                id: generateID(filename),
                fallbackID,
                translationArrayID,
                bindingName,
...
function getTranslationFilePaths(filename, translationDirName) {
  return glob.sync(path.resolve(path.dirname(filename), translationDirName, '*.json'), {
    nodir: true
  });
...
export { I18N_CALL_NAMES, injectWithI18nArguments as default };
