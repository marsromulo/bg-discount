...
import { I18nContext, I18nIdsContext, I18nParentContext } from './context.mjs';
...
function useComplexI18n({
  ...
  const parentIds = React.useContext(I18nIdsContext); // Parent IDs can only change when a parent gets added/ removed,
  ...
  const ids = useLazyRef(() => id ? [id, ...parentIds] : parentIds); // When the manager changes, we need to do the following IMMEDIATELY (i.e.,
  ...
  if (manager !== managerRef.current) {
    ...
    unsubscribeRef.current = manager.subscribe(ids.current, ({
      ...
      const newI18n = new I18n(translations, _objectSpread2(_objectSpread2({}, details), {}, {
        loading
      }));
  ...
  const [i18n, setI18n] = React.useState(() => {
    ...
    return new I18n(translations, _objectSpread2(_objectSpread2({}, manager.details), {}, {
      loading
    }));
  });
  ...
  const shareTranslationsComponent = useLazyRef(() => function ShareTranslations({
    ...
    return /*#__PURE__*/React.createElement(I18nIdsContext.Provider, {
      value: ids.current
    }, /*#__PURE__*/React.createElement(I18nParentContext.Provider, {
...
function useSimpleI18n(manager) {
  const i18n = React.useContext(I18nParentContext) || new I18n([], manager.details);
